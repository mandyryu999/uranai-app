...
# ADMIN_HTML constant, add navigation buttons by replacing one exact,
...
_CHART_HEAD_WITH_RESULT = '<div class="card"><div class="card-head"><h2>算命学命式</h2><div><button class="gold small" onclick="if(selectedId)window.location.href=\'/admin/clients/\'+selectedId+\'/sanmeigaku-result\';else alert(\'相談者を選択してください\')">鑑定結果を見る</button> <button class="gold small" onclick="if(selectedId)window.location.href=\'/admin/clients/\'+selectedId+\'/sanmeigaku-report\';else alert(\'相談者を選択してください\')">総合鑑定レポートを見る</button> <button class="secondary small" onclick="openChartModal()">登録・編集</button></div></div><div id="chartCard"></div></div>'

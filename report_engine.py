...
def build_sanmeigaku_report(chart) -> dict:
    ...
    life_flow = [
        {"title": "初年期", "star": early.get("name") if early else None, "text": _sentence(early), "period_text": early.get("period_meaning") if early else None, "keywords": early.get("keywords", []) if early else []},
        {"title": "中年期", "star": middle.get("name") if middle else None, "text": _sentence(middle), "period_text": middle.get("period_meaning") if middle else None, "keywords": middle.get("keywords", []) if middle else []},
        {"title": "晩年期", "star": late.get("name") if late else None, "text": _sentence(late), "period_text": late.get("period_meaning") if late else None, "keywords": late.get("keywords", []) if late else []},
    ]
    ...
    if middle:
        overview_parts.append(f"人生の中心期には{middle['name']}のエネルギーが示され、{middle.get('summary', '')}")

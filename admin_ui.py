...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}body{margin:0}button,input,textarea,select{font:inherit}button{cursor:pointer}
    ...
    .modal-bg{position:fixed;inset:0;background:#0007;display:none;align-items:center;justify-content:center;padding:20px;z-index:50}.modal{background:#fff;width:min(760px,100%);max-height:90vh;overflow:auto;border-radius:16px;padding:20px}.modal h2{margin-bottom:14px}.form-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}.form-grid .full{grid-column:1/-1}.label{font-size:12px;color:#6b6258;margin-bottom:5px}.actions{display:flex;justify-content:flex-end;gap:8px;margin-top:16px}.chart-layout{display:grid;grid-template-columns:1fr 1fr 1fr;grid-template-areas:". north early" "west center east" "late south middle";gap:8px;margin-top:10px}.starbox{border:1px solid #e4ddcf;border-radius:10px;padding:10px;text-align:center;background:#fbfaf7}.starbox b{display:block;font-size:11px;color:#7a6d5b;margin-bottom:3px}.jusei{background:#f7f1e5;border-color:#d9c59c}.center{grid-area:center}.north{grid-area:north}.east{grid-area:east}.south{grid-area:south}.west{grid-area:west}.early{grid-area:early}.middle{grid-area:middle}.late{grid-area:late}
